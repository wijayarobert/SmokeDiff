...
def main():
    ...
    for _ in range(len(data)):
        ...
        vutils.save_image(ensres, fp = os.path.join(args.out_dir, str(slice_ID)+'_output_ens'+".jpg"), nrow = 1, padding = 10)
         ...
         # Apply threshold to the ensemble result
        threshold = 0.5
        binary_mask = (ensres > threshold).float()
        vutils.save_image(binary_mask, fp=args.out_dir + str(slice_ID) + '_output_binary_mask' + ".jpg", nrow=1, padding=10)
